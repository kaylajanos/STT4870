...
\begin{figure}[h]
<<GraphShow, echo = FALSE, fig.width =3.5, fig.height = 3.5, out.width='.5\\textwidth'>>=
<<Graph>>
@
...
<<molatex, tidy = FALSE, eval = FALSE, rcode = TRUE>>=
...
<<molatexFIG, echo = FALSE, out.width='1.0\\textwidth'>>=
f <- function(x){sqrt(2/(x - 1))*gamma(x/2)/gamma((x - 1)/2)}
library(ggplot2)
p <- ggplot(data.frame(x = c(2, 50)), aes(x = x))
p + stat_function(fun = f) + 
...
p + geom_point(size = 1, color = "blue") + theme_bw() + geom_smooth(method = "lm") + 
