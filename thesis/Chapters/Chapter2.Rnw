...
Typing the code in \gref{RepoCheck} after entering the code in \gref{UPSTREAM} should return something similar to the following:

<<label = OriginUpstream, engine = "bash", eval = FALSE>>=
origin  https://github.com/Your-User-Name/STT4870.git (fetch)
origin	https://github.com/Your-User-Name/STT4870.git (push)
