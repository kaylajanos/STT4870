...
<<label= BashCheck, engine="R", cache = TRUE>>=
Sys.which("bash")
@
...
<<label = GITsetup, eval = FALSE, engine = "bash">>=
...
<<verify, engine = "sh", GIT = TRUE>>=
git config --list  # shows your configuration
@

From the \verb|credential.helper| line in Git Example \vref{verify}, 
one can see the 
\verb|credential.helper| is being used.  Now that Git is set up on your computer, we provide instructions for setting up a remote repository on GitHub.
...
\href{https://education.github.com}{https://education.github.com}.  A step-by-step guide 
for setting up your GitHub organization for classroom use can be found at 
\href{https://education.github.com/guide}{https://education.github.com/guide}.   Free private repositories for anyone can be created at 
...
Point your browser to \href{https://github.com}{https://github.com};
type a username in the \texttt{Pick a username} box (please use \texttt{firstlast}, for example my 
username is \texttt{alanarnholt}); enter your email (use your school email) in the 
\texttt{Your email} box; type in your password in the \texttt{Create a password} box. Then, 
click the \texttt{Sign up for GitHub} box, and you will have a GitHub account.  You should 
use the same name you used when you set up your \texttt{user.name} and \texttt{user.email} 
...
then, give your repository a name and optionally a description (\vref{CreateGR}.)  When you finish, click 
the \texttt{Create repository} button, and your GitHub repository will be created.  You should click in the box \texttt{Initialize this repository with a README} if you want GitHub to create a markdown README file.
...
To keep track of this repo, you need to add another remote named \texttt{upstream.}  This can be done by typing

<<label = UPSTREAM, eval = FALSE, engine = "bash">>=
...
<<label = STATUS, engine = "bash">>=
git status
@
The \texttt{git status} shows us what files are not staged for a commit.  Before files can be
committed, they must be added to the staging area.  Files are added to the stating area
with the command \verb|git add file_name.|  To add all files in the working directory, one
can use \verb|git add .| (The command includes the period.)  Next, all files are added to the staging area, and a snapshot is 
...
Now, just to show how cool this is, we will mix in a little \texttt{R.}


\clearpage
\section{Using R}

<<label = Rgraph, engine = 'R', fig.width = 8>>=
library(ggplot2)
...
I love graphs!  The following graph created with \texttt{ggplot2} uses Greek letters in
the facet panels.  
...
\begin{figure}[!ht]
<<G2, comment = NA, out.width=".65\\linewidth",fig.align='center', echo = FALSE, dev = "tikz",fig.height=5,fig.width=5>>=
hist(rnorm(1000), col = "green", breaks = "Scott", freq = FALSE)
@
\caption{Something goes here \label{graph2}}
\end{figure}
...
\section{So you want to collaborate?}
