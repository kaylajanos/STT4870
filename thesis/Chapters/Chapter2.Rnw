...
The result from entering the code in \gref{RepoCheck} should return the following as a minimum.  That is, you may have 

<<repocheckANS, engine = "bash", eval = FALSE>>=
...
Typing the code in \gref{RepoCheck} after entering the code in \gref{UPSTREAM} should return something similar to \gref{OriginUpstream}.
...
Note that changes you make will not be made to the source repository unless the project maintainer ``pulls'' your changes after you make a pull request.  Pull requests are a way to notify the project maintainer about changes in your fork of their respository.  To initiate a pull request see \sref{SUWTC}.
