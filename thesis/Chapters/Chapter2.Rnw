...
<<label=verify, engine = "bash", GIT = TRUE>>=
git config --list  # shows your configuration
@

From the \verb|credential.helper| line in Git Code \vref{verify}, one can see the 
...
\begin{figure}[!ht]
<<G2, comment = NA, out.width=".95\\linewidth",fig.align='center', echo = FALSE>>=
