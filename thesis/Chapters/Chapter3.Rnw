...
Now, just to show how cool this is, we will mix in a little \texttt{R.}  First, consider the following graph where the \texttt{R} code that created the graph is shown to the reader.

<<label = Rgraph, engine = 'R', fig.width = 5, out.width=".5\\textwidth", fig.align='center'>>=
library(ggplot2)
...
\fref{graph2} uses the \texttt{dev = "tikz"} option so that the fonts in the graph are in agreement with the document fonts.
