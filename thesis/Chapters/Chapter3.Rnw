...
Now, just to show how cool this is, we will mix in a little \texttt{R.}  First, consider the following graph where the \texttt{R} code that created the graph is in the document but not shown to the reader.
