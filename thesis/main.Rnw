...
\documentclass[11pt, letterpaper, oneside]{Thesis} % Paper size, default font size and one-sided paper  % change a4paper to letterpaper ATA letterpaper, 

\usepackage[letterpaper,includehead,includefoot]{geometry}
\geometry{top=2.0in, bottom=0.0in, left=2.25in, right=0.0in}
\usepackage{pdfpages}
...
\emph{Author:}\\
\href{https://github.com/alanarnholt/SEMINAR}{\authornames} % Author name - remove the \href bracket to remove the link
\end{flushleft}
\end{minipage}
\begin{minipage}{0.4\textwidth}
\begin{flushright} \large
\emph{Supervisor:} \\
\href{http://www1.appstate.edu/~arnholta/}{\supname} % Supervisor name - remove the \href bracket to remove the link  
\end{flushright}
\end{minipage}\\[3cm]
